...
def get_target_picture(file_name, label_path, databases='ck+'):
    if databases=='ck+':
        labels = glob.glob(os.path.join(label_path,file_name[:-7]+"*.png"))
        if labels==[]:
            print(os.path.join(label_path, file_name[:-7],"*.png"))
        else:
            return labels[0]
...
def get_dataset(seed = 1234):
    #datasets_path = r'C:\Users\zdc\Downloads\FER\dataset'
    datasets_path = r'.\dataset'
    dataset_name = 'CK+ rearranged rotated cropped5'
